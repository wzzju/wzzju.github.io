...
> python的代码补全依赖`jedi-vim`插件，其需要预先安装`jedi`库，可使用`pip install jedi`完成安装。若使用`virtualenv`进行python环境控制，需要安装vim-virtualenv插件以正确设置`PYTHONPATH`，否则VIM在初始化`jedi-vim`插件时会报错。
...
" disable the long line wrapping in markdown files,
" required by the glow markdown previewer
autocmd FileType markdown set nowrap
autocmd FileType markdown set sidescroll=1
