...
### 3.4 安装vim-virtualenv插件
在`~/.vim_runtime/my_plugins.vim`中添加`Plug 'jmcantrell/vim-virtualenv'`，保存后再次打开VIM运行`:PlugInstall`命令完成安装。

* 当使用`virtualenv`进行python环境控制时，需要安装vim-virtualenv插件以正确设置`PYTHONPATH`，否则VIM在初始化`jedi-vim`插件时会报错
  - `jedi-vim`插件依赖`jedi`库，可使用`pip install jedi`进行安装
* `virtualenv`的安装与使用：
...
* VIM命令模式中查看使用的python路径：`:py3 import sys; print(sys.executable)`
* VIM命令模式中查看使用的python包路径：`:py3 import sys; print(sys.path)`
...
--extra=+q
```
> * `--c++-kinds=+p`表示为标签添加函数原型信息
> * `--fields=+iaS`表示为标签添加继承信息、访问控制信息和函数特征（函数签名,如参数列表等）
> * `--extra=+q`表示为类成员标签添加类标识。这些选项可为`OmniCppComplete`提供更多提示信息
