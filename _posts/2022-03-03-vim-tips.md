...
### 3.6 安装markdown预览插件
在`~/.vim_runtime/my_plugins.vim`中添加`Plug 'skanehira/preview-markdown.vim'`，保存后再次打开VIM运行`:PlugInstall`命令完成安装。
...
nmap <F4> :PreviewMarkdown right<CR>
```
配置完成后，使用`F4`即可对markdown文件进行预览。
> `~/.vim_runtime/my_configs.vim`的完整内容详见<a href="#my_configs">此处</a>。
...
```
> 使用`clangd --version`验证clangd可以正常运行。

#### 3.7.2 安装pylsp
使用`pip install python-lsp-server`命令安装python语言服务器协议，并将`pylsp`所在路径加入到`PATH`搜索路径中，举例如下：
```shell
...
```
> 使用`pylsp --version`验证pylsp可以正常运行。
...
> **备注：**
> * `ctrl + o`可将光标跳转到上一个时间点停留的位置
> * `ctrl + i`可将光标跳转到下一个时间点停留的位置
> * `,l`可将光标在最前一个时间点与最后一个时间点的停留位置之间进行切换
> * 更多`vim-lsp`支持的命令详见[Supported commands](https://github.com/prabirshrestha/vim-lsp#supported-commands)
...
" disable the long line wrapping
set nowrap
set sidescroll=1
