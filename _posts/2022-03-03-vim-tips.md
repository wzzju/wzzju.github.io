...
#### 3.4.2 使用yapf格式化python代码

使用`pip install yapf`安装yapf，并在`~/.vim_runtime/my_configs.vim`文件中添加如下内容：

```viml
" use yapf to format python files
autocmd FileType python nnoremap <Leader>y :0,$!yapf --style google<Cr><C-o>
autocmd BufWritePre *.py 0,$!yapf --style google
```
