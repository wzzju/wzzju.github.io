...
默认情况下，`clangd`会自动在源码文件所处目录下、父目录下以及每层目录的build文件夹下寻找`compile_commands.json`。如果构建目录名称不为`build`，可通过执行类似`ln -s ~/project/build_dir/compile_commands.json ~/project/`的命令创建软连接解决。
