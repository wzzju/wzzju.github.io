...
def showTips():
    ...
    print('-------->> You can visit the URL:   http://{}:{}'.format(get_host_name(), str(port)))
...
def modification_date(filename):
    ...
    file_time = time.localtime(os.path.getmtime(filename)) if filename else time.localtime()
...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        print("Post flag: {}. {} By: {}".format(r, info, self.client_address))
        f = StringIO()
    ...
    def list_directory(self, path):
        ...
        for name in list:
            ...
            try:
                ...
                f.write('<table><tr><td width="60%%"><a href="%s">%s</a></td><td width="20%%">%s</td><td width="20%%">%s</td></tr>\n'
                        % (urllib.quote(linkname), colorName,
                            sizeof_fmt(0), modification_date("")))
        f.write("</table>\n<hr>\n</body>\n</html>\n")
        length = f.tell()
        f.seek(0)
        self.send_response(200)
